angular.module('simapp').factory('AttributeDescriptor', (DataCache,$http,ModelFactory,ObjectFactory) ->
  ...
  modelMethods = ModelFactory("attribute_descriptors", addMethods, { url_prefix: "admin/" })

  # create the custom model methods
  modelMethods.simulation_attribute_descriptors = ->
    ret = {}
    angular.forEach DataCache.attribute_descriptors, (attr_desc, id) ->
  ...
  modelMethods["promise"] = $http.get('admin/attribute_descriptors')
    .success (data, status, headers, config) ->
      angular.forEach data.attribute_descriptors, (attribute_descriptor) ->
        DataCache.attribute_descriptors[attribute_descriptor.id] = attribute_descriptor
        DataCache.attribute_descriptors[attribute_descriptor.id].editing = false
      angular.forEach DataCache.attribute_descriptors, (attribute_descriptor, attribute_descriptor_id) ->
        addMethods(attribute_descriptor)
